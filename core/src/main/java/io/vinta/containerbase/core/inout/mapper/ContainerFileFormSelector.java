...
public enum ContainerFileFormSelector {
	...
	TRANSPORT_EQUIPMENT_TYPE("transportEquipmentType", container ->
			Optional.ofNullable(container.getTransportEquipmentType())
					.map(Enum::name)
					.orElse(null)),
	TARE_WEIGHT_KG("tareWeightKg", container ->
			Optional.ofNullable(container.getTareWeightKg())
					.map(Object::toString)
					.orElse(null)),
	MAX_GROSS_WEIGHT_KG("maxGrossWeightKg", container ->
			Optional.ofNullable(container.getMaxGrossWeightKg())
					.map(Object::toString)
					.orElse(null)),
